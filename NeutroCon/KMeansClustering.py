...
def KMeansClustering(context: np.ndarray, n_clusters: int, max_iters: int = 100, tol: float = 1e-4) -> tuple:
    ...
    # 1D Matrix
        ...
        centroids = np.random.choice(context, n_clusters, replace=False)
        for iteration in range(max_iters):
            distances = np.abs(context[:, np.newaxis] - centroids)
            labels = np.argmin(distances, axis=1)
            new_centroids = np.array([context[labels == i].mean() for i in range(n_clusters)])
            if np.all(np.abs(new_centroids - centroids) < tol):
                break
            centroids = new_centroids
        projected_matrix = np.column_stack((context, labels))
